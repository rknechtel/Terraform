...
<#
.SYNOPSIS
    File Name: install-az-modules.ps1
    ...
    sudo /usr/bin/pwsh -NoLogo -NoProfile -NonInteractive -Command . '$(Build.Repository.LocalPath)/scripts/install-az-modules.ps1'
#>

<#
Azure Pipelines command line task (YAML)
steps:
- task: CmdLine@2
  inputs:
